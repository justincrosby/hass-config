...
2. **Use double quotes on the outside, single quotes on the inside for nested strings**
   - Correct: `state_trigger="media_player.entity == 'on'"`
   - Incorrect: `state_trigger='media_player.entity == "on"'`
3. Don't reassign `pyscript` or `state` variables
4. Keep all code in the main event loop (don't spawn threads calling pyscript functions)
5. For complex callbacks or special methods, prefer native Python imports
6. Use `@pyscript_compile` when you need true Python behavior for callbacks or special methods
